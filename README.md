...
O usuario precisa autenticar no LDAP e pertencer ao grupo `SCAN`. A validacao tenta `memberOf` e, quando `LDAP_REQUIRED_GROUP_DN` estiver definido, tambem consulta o grupo pelo atributo `member`, incluindo grupos aninhados no Active Directory. Para diagnostico temporario, use `AUTH_DEBUG=true` e acompanhe `journalctl -u scan-dominio -f`.
